...
function parseUI5DeployYaml() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      !trimmedLine.includes("ui5-task-zipper")
    ) {
      inZipperConfig = false;
    }
  }
  ...
  if (!archiveName) {
    throw new Error(
      "Could not find archiveName in ui5-task-zipper configuration",
    );
  }
...
function getAppInfoFromManifest() {
  ...
  if (!fs.existsSync(manifestPath)) {
    console.warn(
      "Warning: webapp/manifest.json not found, service name will not be updated",
    );
  ...
  try {
    ...
    if (!serviceName)
      throw new Error("sap.cloud.service not defined in manifest.json");
...
function updateComponentPreload(content, serviceName, replacements) {
  try {
    ...
    modifiedContent = content.replace(
      ...
      (match, key, quotedValue, singleQuoted, doubleQuoted, comma) => {
        try {
          ...
          console.warn(
            `Warning: Could not parse embedded manifest in Component-preload.js: ${error.message}`,
          );
    ...
    ({ modifiedContent, hasChanges } = applyReplacements(
      modifiedContent,
      replacements,
    ));
    ...
    console.warn(
      `Warning: Could not update embedded manifest: ${error.message}`,
    );
      ...
      (async () => {
        try {
          ...
          if (fileName === "manifest.json") {
            ...
            const replacementResult = applyReplacements(
              modifiedContent,
              replacements,
            );
            ...
            const result = updateComponentPreload(
              content,
              serviceName,
              replacements,
            );
